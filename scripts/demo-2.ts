/**
 * THE SEPOLIA DEMO. A standalone copy of `scripts/demo.ts`, differing ONLY in the
 * three amounts below.
 *
 * !! DUPLICATED ON PURPOSE, AND THAT HAS A COST. !! Any fix to the eleven
 * transactions, the assertions, or the allowance handling must be applied to BOTH
 * files or they will silently disagree about what the demo proves. Diff them before
 * trusting either:
 *
 *     diff <(sed 1,40d scripts/demo.ts) <(sed 1,52d scripts/demo-2.ts)
 *
 * Why it exists: Base Sepolia cannot supply DESIGN's figures. Circle's faucet
 * dispenses 20 USDC per address per 2 hours and Coinbase CDP's 10 per day, so
 * funding a 500-USDC deposit is ~50 hours of claiming. `demo.ts` keeps DESIGN's
 * 1,000 / 500 / 600 for fork rehearsals, where USDC is minted rather than begged;
 * this file scales all three by 50 for the live testnet run.
 *
 * Scaling preserves every relationship the eleven transactions assert -- the deposit
 * is half the cap, and the attempt exceeds the remainder by the same proportion --
 * so `ExceedsMandate(remaining, attempted)` still reports the operands the criterion
 * names. Only the units shrink. Recorded as a deviation in the group 5 note.
 *
...
// DESIGN's 1,000 / 500 / 600, scaled by 50 to fit one Circle faucet claim.
const CAP = 20_000_000n; // 20 USDC at 6dp
const FIRST_DEPOSIT = 10_000_000n; // 10 USDC -- half the cap, as in DESIGN
const OVER_CAP = 12_000_000n; // 12 against a remaining 10
